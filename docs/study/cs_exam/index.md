...
![Pasted image 20260114185621.png](../../images/Pasted%20image%2020260114185621.png)
...
![Pasted image 20260114190015.png](../../images/Pasted%20image%2020260114190015.png)

!!! note
    解析
    0x10-0x24=-0x14=-20，然后由于b型指令的偏移计算是imm\*2，所以填入的立即数是-10，换算为十六进制应该是0xFF6

![Pasted image 20260114190055.png](../../images/Pasted%20image%2020260114190055.png)

!!! note
    解析
    jr 指令的语义是无条件跳转到寄存器中给出的地址。RISC-V 中没有 jr，但可以用 jalr 指令实现同样功能，因为 jalr 以寄存器加立即数作为跳转目标地址。**正确选项为 B。**

!!! tip
    `reg`一般代表寄存器，于是和寄存器有关的缩写大多带有`r`，可以凭借这个进行判断

![Pasted image 20260114190217.png](../../images/Pasted%20image%2020260114190217.png)

!!! note
    解析
    循环中分支行为为连续 9 次跳转，随后 1 次不跳转，模式周期为 10。
...
![Pasted image 20260114190648.png](../../images/Pasted%20image%2020260114190648.png)

![Pasted image 20260114190654.png](../../images/Pasted%20image%2020260114190654.png)

!!! note
    解析
    在单周期 CPU 中，所有指令都必须在一个时钟周期内完成，所以时钟周期要足够长以覆盖“最慢指令”的组合逻辑延迟，时钟频率就由最慢指令决定。四个选项里通常最慢的是访存指令，且 load 一般比 store 更慢，因为 load 需要从存储器读出数据并回写寄存器，而 store 不需要回写寄存器，路径更短。因此由 ld 的执行时间决定。**答案选 A。**

![Pasted image 20260114190848.png](../../images/Pasted%20image%2020260114190848.png)

!!! note
    解析
    这是一条典型的无前递、无冒险检测的 5 级流水数据相关插入 NOP 问题。关键规则是寄存器在同一周期前半写、后半读，所以只要消费者指令的读寄存器阶段 ID 与生产者指令的写回阶段 WB 落在同一周期也可以读到新值；否则就必须把消费者的 ID 推迟到不早于生产者的 WB。
...
![Pasted image 20260114191213.png](../../images/Pasted%20image%2020260114191213.png)
...
![Pasted image 20260114191608.png](../../images/Pasted%20image%2020260114191608.png)

!!! note
    解析
    平均读时间约等于平均寻道时间加平均旋转等待时间加数据传输时间。
...
![Pasted image 20260114192423.png](../../images/Pasted%20image%2020260114192423.png)

!!! note
    解析
    先算 Cache 中用于存数据的位数。能存放 4K 字数据，1 字为 32 位，因此数据区大小为 4096×32=131072 位，即 128K 位。
...
![Pasted image 20260114194258.png](../../images/Pasted%20image%2020260114194258.png)

![Pasted image 20260114194308.png](../../images/Pasted%20image%2020260114194308.png)

!!! note
    解析
    已知主频 1GHz，也就是每秒 10^9 个时钟周期，执行周期数 = 执行时间 × 10^9，CPI = 执行周期数 / 动态指令数。
...
![Pasted image 20260114195748.png](../../images/Pasted%20image%2020260114195748.png)

!!! note
    解析
    先从“寄存器含义”开始反推。按照 RISC-V 调用约定，**x10 是第 1 个参数，x11 是第 2 个参数**，所以函数形参一定是 `funct(int x, 另一个参数 y)`。再看循环里每次对“当前地址”做 `addi x6, x6, 8`，说明 y 指向的元素步长是 8 字节，也就是 long long（或 64 位整数），因此 (1) 应是`long long int *`，形参为 `long long int *y`。
...
`add x8, x0, x0` 把 x8 清零，后面在循环中出现 `addi x8, x8, 1` 表示每次满足某条件就自增一次，因此 C 里 int t = (2) 应为 0，并且 (6) 对应`t++`，最后 Exit 处 `add x10, x8, x0` 把 x8 放到返回寄存器 x10，说明 return (8) 是 t。
...
![Pasted image 20260114201049.png](../../images/Pasted%20image%2020260114201049.png)

!!! note
    解析

1、**把两个数用规格化科学计数法二进制表示**：$12.75_{10}=1100.11_2=1.10011×2^3$；$6.5_{10}=110.1_2=1.101×2^2$
2、**对阶**： $1.101×2_2=0.1101×2^3$
3、**尾数相加**： $1.10011+0.1101=10.01101$
4、**结果规格化，并检查溢出和舍入**：$10.01101×2^3=1.001101×2^4$，没有溢出，无须舍入
5、**阶码移码**：$4+15=19$（$10011_2$），正数符号位：$0$，尾数 $001101$，隐藏 $1$
6、**二进制位模式和十六进制数**：$0 10011 0011 0100 00$；$0X 4CD0$

![Pasted image 20260114201725.png](../../images/Pasted%20image%2020260114201725.png)

![Pasted image 20260114201732.png](../../images/Pasted%20image%2020260114201732.png)
...
-**ALUSrc**：`sd`指令的源操作数来自寄存器 x22 和常数8，所以ALUSrc为1
- 看图中ALUSrc的部分，是一个多路选择器，当有ImmGen输入的时候就有1，所以为1
-**ALUOp**：内存访问指令，需要计算地址，所以ALUOp应该设置为进行加法运算，即 `00`（答案还给了`add`，估计为了捞捞）
-**PCSrc**：本指令不涉及跳转，所以为0
...
![Pasted image 20260114204957.png](../../images/Pasted%20image%2020260114204957.png)
...
![Pasted image 20260114210509.png](../../images/Pasted%20image%2020260114210509.png)

![Pasted image 20260114210516.png](../../images/Pasted%20image%2020260114210516.png)
我院课本未出现相关知识点，不做解析

![Pasted image 20260114211402.png](../../images/Pasted%20image%2020260114211402.png)
...
![Pasted image 20260114212755.png](../../images/Pasted%20image%2020260114212755.png)
![Pasted image 20260114212805.png](../../images/Pasted%20image%2020260114212805.png)
...
A是虚拟页号(x单元楼)，应该用虚拟地址位数减去页内偏移位，即 $32-13=19$ 位
B是虚拟页号标记位，是和A进行比较的，则应该为 $19$ 位
...
![Pasted image 20260114222403.png](../../images/Pasted%20image%2020260114222403.png)
...
![Pasted image 20260114222807.png](../../images/Pasted%20image%2020260114222807.png)
...
![Pasted image 20260114223340.png](../../images/Pasted%20image%2020260114223340.png)

![Pasted image 20260114223345.png](../../images/Pasted%20image%2020260114223345.png)

!!! note
    解析
    **1** ALU和通用寄存器都是按字处理的，计算机M的字长为 16 位，那么ALU的位数也为 16 位

按照字节编址，地址线有 20 位，那么可寻址空间有 $2^{20}=1\mathrm{MB}$
指令长度有 16 位，那么指令寄存器有 16 位
...
01B2H=0000000110110010。高6位000000为R型；rs=01对应R1，rt=10对应R2，rd=11对应R3，op1=0010表示带符号整数减法，所以功能是`R3←R1-R2`

01B3H仅最后4位变为0011，表示带符号整数乘法，所以功能是`R3←R1×R2`。
...
![Pasted image 20260114224609.png](../../images/Pasted%20image%2020260114224609.png)
...
规格化：$-0.11001011×2^6=-1.1001011×2^5$，对应数值$-57.375$。
此时$S=1$，$E=5+127=132=10000100$，$M=10010110000000000000000$，
所以Z的IEEE754单精度十六进制为0xC2658000。

![Pasted image 20260114225405.png](../../images/Pasted%20image%2020260114225405.png)
...
![Pasted image 20260114225617.png](../../images/Pasted%20image%2020260114225617.png)
...
![Pasted image 20260114230130.png](../../images/Pasted%20image%2020260114230130.png)
...
![Pasted image 20260114231428.png](../../images/Pasted%20image%2020260114231428.png)
先改成RISCV：
改写后的 RISC-V 指令序列为
...
```
loop:
add x6, x19, x19 add x6, x6, x6 add x6, x6, x22 lw x5, 0(x6) bne x5, x21, exit add x19, x19, x20 jal x0, loop exit:
```
...
**(1)**
第1条写 x6，第2条读 x6，属于第1条与第2条关于 x6 的 RAW 相关。
...
**(4)**
采用 EX/MEM、MEM/WB 转发后，第1、2、3条算术指令之间的相关都可以通过转发完全消除，不需要 nop。
...
![Pasted image 20260114231932.png](../../images/Pasted%20image%2020260114231932.png)
![Pasted image 20260114231939.png](../../images/Pasted%20image%2020260114231939.png)
...
![Pasted image 20260115150148.png](../../images/Pasted%20image%2020260115150148.png)
...
$$
\begin{align}
&\text M1运行P1用时\ &t1=10000ms=10s \\
&\text M1每秒有时钟周期\ &800\text MHZ\ =800\times 10^6 \\
&\text M1运行P1总共用了时钟周期为\ &800\times 10^6(c/s)\times 10s=8\times 10^9 \\
&\text 答案\ &CPI=\dfrac{8\times 10^9}{200\times 10^6}=40
\end{align}
$$
...
![Pasted image 20260115154306.png](../../images/Pasted%20image%2020260115154306.png)
...
![Pasted image 20260115155847.png](../../images/Pasted%20image%2020260115155847.png)
...
`0|110 01|00 0000 0010`**对阶** 大指数为10，小指数为-1，指数差为11
...
`00 0000 0011`**计算答案**`0|110 01|00 0000 0011`=1027
...
![Pasted image 20260115162040.png](../../images/Pasted%20image%2020260115162040.png)
直接看答案吧，课本有解析
![Pasted image 20260115162112.png](../../images/Pasted%20image%2020260115162112.png)
...
![Pasted image 20260115163435.png](../../images/Pasted%20image%2020260115163435.png)
![Pasted image 20260115163447.png](../../images/Pasted%20image%2020260115163447.png)
![Pasted image 20260115163451.png](../../images/Pasted%20image%2020260115163451.png)
...
`loop: slli t1, s3, 2`这里可以看到将 $i\times 4$ 存入 `R[t1]`，说明每次的查询save角标是 4i，则save的每个元素占4字节
...
![Pasted image 20260115173017.png](../../images/Pasted%20image%2020260115173017.png)
...
```
add s3, s1, s0
# s3 = s1 + s0 sub t2, s3, s3
# t2 = s3 - s3 lw t1, 0(t2)
# t1 = Mem[t2] add t3, t1, t2
# t3 = t1 + t2 add t1, s4, s5
# t1 = s4 + s5
```
...
`sub t2, s3, s3`用到了 `s3` 看看它的前面指令有没有 `s3` 作为输出，发现有的，所以这里需要转发
...
`lw t1, 0(t2)`这里将数据加载到t1；`add t3, t1, t2`使用了t1，所以load-use数据冒险发生了

![Pasted image 20260115174651.png](../../images/Pasted%20image%2020260115174651.png)
![Pasted image 20260115174656.png](../../images/Pasted%20image%2020260115174656.png)
...
![Pasted image 20260115175540.png](../../images/Pasted%20image%2020260115175540.png)
![Pasted image 20260115175544.png](../../images/Pasted%20image%2020260115175544.png)
...
1. 根据地址中间3位找到Cache第2组
2. 将标记位与Cache第2组两个Cache行标记位同时比较
3. 如果有一个相等并且有效位为1，则命中。此时根据低9位的块内地址从对应行中取出单元内容送CPU
4. 若不相等或相等但有效位为0，则为命中，此时将主存的第0000 0010 010块复制到第2组的任意空行，并置有效位为1

![Pasted image 20260115180937.png](../../images/Pasted%20image%2020260115180937.png)
...
**算性能提升**：Cache 命中需要 1 周期，主存访问需要 100 周期。
...
![Pasted image 20260115182907.png](../../images/Pasted%20image%2020260115182907.png)
![Pasted image 20260115182916.png](../../images/Pasted%20image%2020260115182916.png)
...
![Pasted image 20260115183258.png](../../images/Pasted%20image%2020260115183258.png)
...
- **数据区容量**: 32 B
- **主存块大小**: 16 B
- **Cache 块数**: 32 B ÷ 16 B = **2 块**

数据类型：
- **数组 x**: 8 个 float = 32 字节，存放在地址 0000 0040H 开始
- **数组 y**: 8 个 float = 32 字节，紧跟在 x 后
...
**数组 x** (从 0x40 开始，32 字节):
- 块0: 0x40-0x47 (x[0], x[1])
- 块1: 0x48-0x4F (x[2], x[3])
- 块2: 0x50-0x57 (x[4], x[5])
- 块3: 0x58-0x5F (x[6], x[7])

**数组 y** (从 0x60 开始，32 字节):
- 块4: 0x60-0x67 (y[0], y[1])
- 块5: 0x68-0x6F (y[2], y[3])
- 块6: 0x70-0x77 (y[4], y[5])
- 块7: 0x78-0x7F (y[6], y[7])
...
访问序列 (只访问 `x[0]~x[7]` 和 `y[0]~y[7]`)
> 注意: 虽然 x 定义为 `float[12]`，但程序只访问 `x[0]~x[7]`
...
**命中率计算**: 总访问次数 16 次，命中次数 12 次，Miss 次数 4 次，命中率 = 12 / 16 = 75%。
